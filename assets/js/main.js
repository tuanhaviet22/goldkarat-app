(function($){
	...
	***********************/
	...
	function customQantity(){
	    $(".quantity-custom").append('<div class="dec qtybutton"><i class="fa fa-angle-down"></i></div><div class="inc qtybutton"><i class="fa fa-angle-up"></i></div>');
...
var slider_about = new Swiper('.slide-about', {
    loop: true,
    pagination: {
...
$('.list-attribute .item').click(function () {
    ...
    $('.add-to-cart').attr('data-attribute-id',productID)
    $(this).addClass('active');
    if (true){
        $.ajax({
    ...
    $(this).parent().parent().siblings().find('.item').removeClass('active')
    $(`.thumb-menu-item a[href="#product-large-${productID}_attribute"]`).trigger('click');
...
function updateProduct(data) {
    $('.product-details-name').text(data.product_name)
    $('#text-quantity').text(data.product_quantity)
    if (data.product_price_sale === null){
        let html = `
            <span class="money">${formatter.format(data.product_price)}</span>
        `;
        $('.product-details-price-wrapper').html(html)
    }else{
        let oldPrice = formatter.format(data.product_price);
        let newPrice = formatter.format(data.product_price_sale);
        let html = `
            <span class="money">${newPrice}</span>
            <span class="product-price-old">
                <span class="money">${oldPrice}</span>
            </span>
        `;
        $('.product-details-price-wrapper').html(html)
    }
